...
This script scans an input directory (recursively) for JSON files that match
train/test patterns (by default, filenames containing ``train`` or ``test``)
and concatenates them into two aggregated JSONL outputs. It assumes each
source file is structured with one JSON object per line, as in ``test_0.json``.
...
def merge_jsonl(files: Iterable[Path], output_path: Path, *, validate_json: bool = False) -> int:
    ...
    Args:
        ...
        validate_json: If True, ensure every non-empty line is valid JSON.
                ...
                for line in infile:
                    ...
                    if validate_json:
                        try:
                            ...
                            raise SystemExit(f"Invalid JSON in {location}: {exc.msg}") from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--train-pattern",
        default="**/*train*.json",
        help="Glob pattern for train files inside the input directory (recursive).",
    )
    parser.add_argument(
        "--test-pattern",
        default="**/*test*.json",
        help="Glob pattern for test files inside the input directory (recursive).",
    )
    parser.add_argument(
        "--output-train",
        type=Path,
    ...
    parser.add_argument(
        "--validate-json",
        action="store_true",
        help="Validate that each non-empty line in the inputs is valid JSON before writing.",
    )
...
def main() -> None:
    ...
    train_count = merge_jsonl(train_files, args.output_train, validate_json=args.validate_json)
    test_count = merge_jsonl(test_files, args.output_test, validate_json=args.validate_json)
